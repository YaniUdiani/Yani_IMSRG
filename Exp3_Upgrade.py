...
#Goes up to 3 terms in BCH expansion, 4 would be better for precision
#3rd Order MBPT turned off
#------------------------------------------------------------------------------
...
#  hermitian = copy.copy(blk_nums_that_matter)
#  #blocks_that_matter=[] #list containing pairs in blocks that will be used in xph    
#  #blk_nums_that_matter=[occphA_2B[row][1] for row in range(len(occphA_2B))]#contain blk# for nonzero blks 
...
#      
#      for block_num in hermitian: #I'm looping over blk_nums_that_matter 
...
#                  
#                  y_ph[block_num][i2, i1] -=y_map[[(c,b),(a,d)]]
#                  y_ph[block_num][i2 + disp_2, i1] -=y_map[[(d,b),(a,c)]]
#                  y_ph[block_num][i2, i1 + disp_1] -=y_map[[(c,a),(b,d)]]
#                  y_ph[block_num][i2 + disp_2, i1 + disp_1] -=y_map[[(d,a),(b,c)]]
...
#def ph_engine1(y_ph,subset2B,y_map_o,disp_1,disp_2,BN,block_num):
#  """ Turns out this is the 2nd most expensive piece of PH_Trans """
#  for i1,(a,b) in enumerate(subset2B[BN]):
#      for i2,(c,d) in enumerate(subset2B[block_num]):
#          y_ph[block_num][i1, i2] -=y_map_o[[(a,d),(c,b)]]
#          y_ph[block_num][i1 + disp_1, i2] -=y_map_o[[(b,d),(c,a)]]
#          y_ph[block_num][i1, i2 + disp_2] -=y_map_o[[(a,c),(d,b)]]
#          y_ph[block_num][i1 + disp_1, i2 + disp_2] -=y_map_o[[(b,c),(d,a)]]  
#  return
...
def ph_transform_2B(x,y,occphA_2B,
                    bas2B,bs_len,subset2B,subset_size,
                    blocks_that_matter,blk_nums_that_matter,y_ph_zeros,
...
#commutator must be such that ~[H,Omega] returns a hermitian matrix; and ~[Omega, eta] returns an anti-hermitian matrix
    ...
    if(type(x) == list and type(y) == list):# 2B-2B
        ...
        xy_ph = ph_transform_2B(x,y,occphA_2B,
                                ...
                                subset_size,btm,bntm,ypz,eng2l,dicty,store1)
...
def main():
  ...
  bn2=Bernoulli_generator(3)# Go up to 3 terms in expansion
