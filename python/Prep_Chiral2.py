...
def List_operation(A, B, block_sizes,sign,operation, bs_len): #performs simple operations on lists
    ...
    if(operation == "Transpose"): #will transpose A and conjugate it
      ...
      for i2, (k,l) in enumerate(bas_block2B[block_num]):
        ...
        if(i1 != i2):
          ...
          if round(x[block_num][i1, i2].imag +sign * x[block_num][i2, i1].imag, 14) != 0.0:
            print(sign)
            print(i1, i2)
            print(x[block_num][i1, i2], x[block_num][i2, i1])
     
            print(round(x[block_num][i1, i2].imag +sign * x[block_num][i2, i1].imag, 14))
                    ...
                    for (k,l) in Particle_P[block_num]:  
                        ...
                        Output_0B += (
                           ...
                           -x[block_num][i2, i1] * y[block_num][i1, i2])
                            #+np.conj(x[block_num][i1, i2]) *np.conj( y[block_num][i2, i1]))
                        #print("why me", x[block_num][i1, i2] * y[block_num][i2, i1], -x[block_num][i2, i1] * y[block_num][i1, i2])
#                        Output_0B += 2*(
#                            x[block_num][i1, i2].real * y[block_num][i2, i1].real
#                            -x[block_num][i1, i2].imag * y[block_num][i2, i1].imag)                        
                        #if(round(x[block_num][i1, i2].imag, 14) != round(-x[block_num][i2, i1].imag, 14)):
    #                    if(round(y[block_num][i1, i2].imag + y[block_num][i2, i1].imag, 14) != 0.0):
                           # print("Why so seriousxr?", round(x[block_num][i1, i2].real, 14), round(x[block_num][i2, i1].real, 14))
                           # print("Why so seriousxm?", round(x[block_num][i1, i2].imag, 14), round(x[block_num][i2, i1].imag, 14))
                           # print("Why so seriousyr?", round(y[block_num][i1, i2].real, 14), round(y[block_num][i2, i1].real, 14))
                           # print("Why so seriousym?", round(y[block_num][i1, i2].imag, 14), round(y[block_num][i2, i1].imag, 14))
       #                 print("Why so seriousdxr?", round(x[block_num][i1, i2].real + x[block_num][i2, i1].real, 14))
      #                  print("Why so seriousdxm?", round(x[block_num][i1, i2].imag - x[block_num][i2, i1].imag, 14))
                        #print("Why so seriousdyr?", round(y[block_num][i1, i2].real - y[block_num][i2, i1].real, 14))
                       # print("Why so seriousdym?", round(y[block_num][i1, i2].imag + y[block_num][i2, i1].imag, 14))
                        if(round(y[block_num][i1, i2].imag + y[block_num][i2, i1].imag, 14) != 0.0):
                            print("Why so seriousdym?", round(y[block_num][i1, i2].imag + y[block_num][i2, i1].imag, 14))
                            sys.exit()
            ...
            for block_num in Hole_A_Filter:# go through blocks
                ...
                for (i,p) in Hole_A[block_num]:
                    ...
                    Output_1B[p] += 0.5 * (xy[block_num][i1, i1] + np.conj(xy[block_num][i1, i1]))
...
#                    Output_1B[r, r] += xyz_map[[(p,r), (p,r),block_num]] 
            ...
            for block_num in bs_len:# go through blocks    
                ...
                for i1, (r,p) in reverse_enumerate(temp_blk,disp):
                    ...
                    Output_1B[p] +=  0.5 * (xyz[block_num][i1, i1] + np.conj(xyz[block_num][i1, i1]))
                    
                    Output_1B[r] += 0.5 * (xyz[block_num][i1 + disp, i1 + disp] + np.conj(xyz[block_num][i1 + disp, i1 + disp]))
                ...
                #Let work =  xyi[block_num][i1, i2]
                 ...
                 #         + xyi[block_num][i1 + disp, i2 + disp])
                  ...
                  #(Wigner generator section)
                        ...
                        #Exploit symmetries/ hermiticity by reusing cache
                        ...
                        Output_2B[block_num][i2, i1] =  np.conj(mval)
                        Output_2B[block_num][i2, i1 + disp] = -np.conj(mval)
                        Output_2B[block_num][i2 + disp, i1] = -np.conj(mval)
                        Output_2B[block_num][i2 + disp, i1 + disp] = np.conj(mval)


                 #       if(i1+disp ==7 and i2==1 or i2+disp == 7 and i1 == 1):
                  #          print(i1+disp, i2, i2+disp, i1) 
                   #         print("fistys")
                    #        print(round(Output_2B[block_num][7, 1].imag,14), round(Output_2B[block_num][1, 7].imag,14))
                     #       if(round(Output_2B[block_num][7, 1].imag,14) != -round(Output_2B[block_num][1, 7].imag,14)):
                      #          print("things are horribly wrong"); sys.exit()
                      #  if(i1 != i2 and round(Output_2B[block_num][i2, i1].imag,14) != -round(Output_2B[block_num][i1, i2].imag,14)):
                        #if(Output_2B[block_num][i2, i1] != np.conj(Output_2B[block_num][i1, i2])):
                       #     print("why")
                        #    print(i1,i2)
                         #   print(sign)
                          #  print(round(Output_2B[block_num][i2, i1].imag,14), round(Output_2B[block_num][i1, i2].imag,14))
                           # print("hermiticity broken", Output_2B[block_num][i2, i1], Output_2B[block_num][i1, i2])
                            #sys.exit() 
                    del ranger[i1] 
...
#                    mval += y[j] * x[block_num][i1 + disp, i2]
                    ...
                    #I hope this is stored in cache
                    mval = x[block_num][i1, i2] * (y[k] + y[l] -y[i] -y[j])
...
#                    print("CM Momentum is not being conserved!")
                ...
                H2B[blocky][block_loc_pq,block_loc_rs] = mval
                ...
                H2B[blocky][block_loc_pq,block_loc_sr]= -mval
                H2B[blocky][block_loc_qp,block_loc_rs]= -mval
                H2B[blocky][block_loc_qp,block_loc_sr]=  mval
                
                #Use Hermiticity, also note that matrix elements are real
                H2B[blocky][block_loc_rs,block_loc_pq]= np.conj(mval)
                H2B[blocky][block_loc_sr,block_loc_pq]= np.conj(-mval)
                H2B[blocky][block_loc_rs,block_loc_qp]= np.conj(-mval)
                H2B[blocky][block_loc_sr,block_loc_qp]= np.conj(mval)
