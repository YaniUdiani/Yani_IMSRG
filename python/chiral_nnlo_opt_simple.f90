module chiral_constants_nnlo_opt
  ...
  !nuclear constants
  ! I'm pulling p & n masses from codata
  ! Neutron mass also wrong in last digit?
  ! real(wp), parameter :: proton_mass  = 938.272_wp   !in [MeV]
  real(wp), parameter :: proton_mass  = 938.2720813_wp   !(58) in [MeV]
  ! real(wp), parameter :: neutron_mass = 939.5653_wp  !in [MeV]
  real(wp), parameter :: neutron_mass = 939.5654133_wp  !(58) in [MeV]
...
! #ifdef ISOSPIN_ON
! making this neutron masses only for neutron matter
    escale = hbarc*(hbarc/(0.5d0*(neutron_mass+neutron_mass)))*mom_scale*mom_scale
! #else
