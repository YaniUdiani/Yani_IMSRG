! takes in raw parameters and quantum numbers and bundles them into the data structures the
! fortran code is expecting

subroutine chipot_f90_wrapper(matel_real,matel_im,npart,rho,ps,pt,ppx,ppy,ppz,qs,qt,qpx,qpy,qpz, &
        rs,rt,rpx,rpy,rpz,ss,st,spx,spy,spz) bind(c)
    ! NNLO_opt chiral interaction
    use chiral_constants_nnlo_opt , only : init_chp_constants_nnlo_opt
    use chiral_potentials_nnlo_opt

    implicit none
    integer, value, INTENT(IN) :: ps,pt,qs,qt,rs,rt,ss,st
    integer, value, INTENT(IN) :: ppx,ppy,ppz
    integer, value, INTENT(IN) :: qpx,qpy,qpz
    integer, value, INTENT(IN) :: rpx,rpy,rpz
    integer, value, INTENT(IN) :: spx,spy,spz
