...
from Box_chiral_elements_asym import H2B_chiral_element
#-----------------------------------------------------------------------------------
# Ploting variables
#-----------------------------------------------------------------------------------
...
def List_operation(A, B, block_sizes,sign,operation, bs_len): #performs simple operations on lists
    ...
    if(operation == "Transpose"): #will transpose A
      return [np.conj(transpose(A[i])) for i in bs_len]
...
def test_hermiticity(x, bas_block2B, sign, bs_len):

  for block_num in bs_len:
        
    for i1, (i,j) in enumerate(bas_block2B[block_num]):
      
      for i2, (k,l) in enumerate(bas_block2B[block_num]):
        
        if(i1 != i2):
        
          if round(x[block_num][i1, i2].real -sign * x[block_num][i2, i1].real, 14) != 0.0:
            
            print("hermiticity is broken at real")
            
            sys.exit()
            
          if round(x[block_num][i1, i2].imag + sign * x[block_num][i2, i1].imag, 14) != 0.0:
            
            print("hermiticity is broken at img")
...
#        y_map_o.obj = y #set appropriate y object
    ...
    #    test_hermiticity(Output_2B, bas_block2B, sign, bs_len)
...
#                        Output_0B += (
 #                           x[block_num][i1, i2] * y[block_num][i2, i1]
  #                          -x[block_num][i2, i1] * y[block_num][i1, i2])
                        
                        Output_0B += 2*(
                            x[block_num][i1, i2].real * y[block_num][i2, i1].real
                            -x[block_num][i1, i2].imag * y[block_num][i2, i1].imag)                        
            ...
            for block_num in Hole_A_Filter:# go through blocks
                ...
                for (i,p) in Hole_A[block_num]:
                    ...
                    Lead = xy[block_num][i1, i1] 
                    
                    Output_1B[p] += 0.5 * (Lead + np.conj(Lead))
...
#                    Output_1B[r, r] += xyz_map[[(p,r), (p,r),block_num]] 
            ...
            for block_num in bs_len:# go through blocks
                ...
                for i1, (r,p) in reverse_enumerate(temp_blk,disp):
                    ...
                    #see Exp_Upgrade_OmegaZ.py in Research_Baby_Steps for 
                    #comments  on why I could simplify those one body expressions.
                    
                    i1_temp_disp = i1 + disp #I hope it's put in a register
                    
                    Argon = temp_xyz[i1, i1] #I hope it's put in a register
                    
                    Neon = temp_xyz[i1_temp_disp, i1_temp_disp]
                                      
                    Output_1B[p] +=  0.5 * (Argon + np.conj(Argon))
                    
                    Output_1B[r] +=  0.5 * (Neon + np.conj(Neon))
                    ...
                    temp_Output_2B[i1, i1_temp_disp]= -np.conj(mval) 
                ...
                #############  Explanation of what is going on below ##########
                #Let work =  xyi[block_num][i1, i2]
                 #         - xyi[block_num][i1 + disp, i2]
                 #         - xyi[block_num][i1, i2 + disp]
                 #         + xyi[block_num][i1 + disp, i2 + disp])
 
                  # Then the actual Output[block_num][i1,i2] =
                  #1/2(xy+sign*transpose(xy))[block_num][i1,i2] - work
                  
                  #Then since it should be symmetrized/have right hermiticty
                  #exploit them. This is slightly obscure, but 
                  #I think that it is right. See Heiko's python IMSRG Code
                  #(Wigner generator section)
                        ...
                        temp_Output_2B[i2, i1] =  np.conj(mval)
                        temp_Output_2B[i2, i1_temp_disp] = -np.conj(mval)
                        temp_Output_2B[i2_temp_disp, i1] = -np.conj(mval)
                        temp_Output_2B[i2_temp_disp, i1_temp_disp] = np.conj(mval)
...
#                    mval += y[j] * x[block_num][i1 + disp, i2]
                    ...
                    temp_Output_2B[i2, i1]= np.conj(mval)
                    temp_Output_2B[i2, i1 + disp]= -np.conj(mval)
                    temp_Output_2B[i2 + disp, i1]= -np.conj(mval)
                    temp_Output_2B[i2 + disp, i1 + disp]= np.conj(mval)
...
def Inf_Matter_Ham(full_state, energy_const, rho, user_data):
    ...
    for blocky in bs_len:# fill in all blocks
        ...
        for (p,q) in subset2B[blocky]: #I could use enumerate 
            ...
            real_val, im_val = H2B_chiral_element(p, q, p, q, full_state, rho)
                
            mval = complex(real_val, im_val) #stored in cache?            
            ...
            temp_H2B[block_loc_qp,block_loc_pq]= -np.conj(mval)
...
#                    print("CM Momentum is not being conserved!")
                ...
                real_val, im_val = H2B_chiral_element(p, q, r, s, full_state, rho)
                ...
                temp_H2B[block_loc_rs,block_loc_pq]= np.conj(mval)
                temp_H2B[block_loc_sr,block_loc_pq]= -np.conj(mval)
                temp_H2B[block_loc_rs,block_loc_qp]= -np.conj(mval)
                temp_H2B[block_loc_sr,block_loc_qp]= np.conj(mval)
                
#                if(round(H2B[blocky][block_loc_pq,block_loc_rs].imag, 14) != round(H2B[blocky][block_loc_rs,block_loc_pq].imag, 14)):
#
#                    print("Why so serious1?", round(x[block_num][i1, i2].real, 14), round(x[block_num][i1, i2].imag, 14))
#                    print("Why so serious2?", round(y[block_num][i1, i2].real, 14), round(y[block_num][i1, i2].imag, 14))
...
def main(args): 
  ...
  parser.add_argument("--comp", required=False, type= int, default=1, choices=[0,1],
  ...
  fast_full_state = sorter(temp_full_state, hole_Nmax)#order the states so that holes come before particle states in the list. I like it that way
  
  full_state = np.array(fast_full_state)#covert it to an array for easy viewing
  ...
  for i in states: # patchwork: make floats into ints for chiral routine
    
    fast_full_state[i][3] = int(2*fast_full_state[i][3])  
  ...
  print("Number of PH Blocks: ",len(basph_block2B))
  ...
  # send fast_full_state into Inf_Ham_calc. No need for numpy arrays
  H1B, H2B=Inf_Matter_Ham(fast_full_state, energy_const, rho, user_data)
  #test_hermiticity(H2B, bas_block2B, +1, bs_len)
  ...
  for flow_p in flow_pars:
    ...
    print("Omega norm: ", calc_full2B_norm(Omega_2B, bs_len)) 
    E_prev = E
    
    Transformed_Hamiltonian = Transformed_Ham(Omega_2B, user_data)#get  Hamiltonian to use MBPT, and print out values
    E, f , Gamma = Transformed_Hamiltonian #get  Hamiltonian to use MBPT, and print out values
   
    if(E.imag != 0.0 or LA.norm(f.imag)!= 0.0):

      print("Something is wrong in the commutator expressions. They're probably hermiticity issues")
      print("E and f need to be all real")
      print("real+ im", E)
      print("norm of img f: ", LA.norm(f.imag))
      sys.exit()   
