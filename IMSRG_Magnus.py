...
def special_commutator(x,y,user_data): # takes in either 1 or 2 body matrices to output 0,1, and 2 body commutators
    ...
    occC_2B   = user_data["occC_2B"]
    occphA_2B = user_data["occphA_2B"]
    states=particles+holes
    Output_1B=np.zeros((dim1B,dim1B))
    Output_2B=np.zeros((dim1B**2,dim1B**2))
    Output_0B=0.0
    # The remaining lines of the function hardcode the commutators. I will convert some of these to matrix multiplication soon
    if(np.size(x)== len(states)**2 and np.size(y)== len(states)**2):#1B-1B
        Output_1B=Output_1B+commutator(x,y)
        for i in holes:# 0B Correction
            for j in particles:
                Output_0B=Output_0B+x[i,j]*y[j,i]-x[j,i]*y[i,j]
            ...
            for j in states:
                for a in holes:
                    for b in particles:
                        ...
                        Output_1B[i,j]=Output_1B[i,j]-(y[i,j]*x[idx2B[(b,i)],idx2B[(a,j)]]-y[i,j]*x[idx2B[(a,i)],idx2B[(b,j)]])
        ...
        for i in states:
            for j in states:
                for k in states:
                    for l in states:
                        for a in states:
                            Output_2B[idx2B[(i,j)],idx2B[(k,l)]]=Output_2B[idx2B[(i,j)],idx2B[(k,l)]]-( (y[i,a]*x[idx2B[(a,j)],idx2B[(k,l)]]-y[j,a]*x[idx2B[(a,i)],idx2B[(k,l)]]) - (y[a,k]*x[idx2B[(i,j)],idx2B[(a,l)]]-y[a,l]*x[idx2B[(i,j)],idx2B[(a,k)]]))
            ...
            for j in states:
                for a in holes:
                    for b in particles:
                        ...
                        Output_1B[i,j]=Output_1B[i,j]-(x[i,j]*y[idx2B[(b,i)],idx2B[(a,j)]]-x[i,j]*y[idx2B[(a,i)],idx2B[(b,j)]])
        ...
        for i in states:
            for j in states:
                for k in states:
                    for l in states:
                        for a in states:
                            Output_2B[idx2B[(i,j)],idx2B[(k,l)]]=Output_2B[idx2B[(i,j)],idx2B[(k,l)]]+( (x[i,a]*y[idx2B[(a,j)],idx2B[(k,l)]]-x[j,a]*y[idx2B[(a,i)],idx2B[(k,l)]]) - (x[a,k]*y[idx2B[(i,j)],idx2B[(a,l)]]-x[a,l]*y[idx2B[(i,j)],idx2B[(a,k)]])) 
            ...
            for j in holes:
                for k in particles:
                    for l in particles:
                        ...
                        Output_0B=Output_0B+0.25*(x[idx2B[(i,j)],idx2B[(k,l)]]*y[idx2B[(k,l)],idx2B[(i,j)]]-x[idx2B[(k,l)],idx2B[(i,j)]]*y[idx2B[(i,j)],idx2B[(k,l)]])                       
        for i in states:
            for j in states:
                for a in states:
                    for b in states:
                        for c in states:
                            Output_1B[i,j]=Output_1B[i,j]+0.5*( x[idx2B[(c,i)],idx2B[(a,b)]]*y[idx2B[(a,b)],idx2B[(c,j)]] - x[idx2B[(a,b)],idx2B[(c,j)]]*y[idx2B[(c,i)],idx2B[(a,b)]] )*((1-int(a in holes))*(1-int(b in holes))*int(c in holes)+(1-int(c in holes))*int(a in holes)*int(b in holes))
...
def RHS_Cal(Omega_1B,Omega_2B,Eta_1B,Eta_2B, user_data):# Generates right hand side of flow equation to be fed to diffy solver to obtain new Omega
    bn2 = user_data["bn2"]
    RHS_0B=0. #Initialize Hamilitonian
    ...
    for n in range(len(bn2)):
        if n==0: 
            ...
            nth_1B=nth_1B+special_commutator(Omega_1B,nth_2B,user_data)[1]
            nth_1B=nth_1B+special_commutator(Omega_2B,nth_1B, user_data)[1]
            nth_1B=nth_1B+special_commutator(Omega_2B,nth_2B, user_data)[1]
            
            nth_2B=special_commutator(Omega_1B,nth_2B, user_data)[2]
            nth_2B=nth_2B+special_commutator(Omega_2B,nth_1B, user_data)[2]
            nth_2B=nth_2B+special_commutator(Omega_2B,nth_2B, user_data)[2]
        ...
        #Higher_Order.append(LA.norm(shiftymatrix_1B))
        #THigher_Order.append(n)
        if ((LA.norm(shiftymatrix_1B)+ LA.norm(shiftymatrix_2B))<1e-10):
            break        
        RHS_0B=RHS_0B+bn2[n]*shiftymatrix_0B
        RHS_1B=RHS_1B+bn2[n]*shiftymatrix_1B
        RHS_2B=RHS_2B+bn2[n]*shiftymatrix_2B

  #      print(LA.norm(shiftymatrix_2B), n)
...
def Transformed_Ham(Omega_1B, Omega_2B, user_data):# Generates new Hamiltonian by performing BCH expansion
    ...
    for n in range(len(bn2)):
        if n==0: 
            nth_2B=Gamma #Initial commutators defined to be Eta
            nth_1B=f
            nth_0B=E
        else:
            # The next lines aggregate the current 0B, 1B, and 2B nth commutators from the Magnus expansion
        ...
        if ((LA.norm(shiftymatrix_1B)+LA.norm(shiftymatrix_2B))<1e-10):
            break
        H_0B=H_0B+shiftymatrix_0B
        H_1B=H_1B+shiftymatrix_1B
        H_2B=H_2B+shiftymatrix_2B
  #      print(LA.norm(shiftymatrix_2B), n)
...
def derivative_wrapper(t, y, user_data):
  ...
  calc_rhs  = user_data["calc_rhs"]    #right hand side of zero body piece
...
def main():
  ...
  bn2=Bernoulli_generator(16)#caveman coding for now
  ...
  user_data  = {
    ...
    "special_commutator": special_commutator
  }
  ...
  while solver.successful() and solver.t < sfinal:
    ...
    dim2B = dim1B*dim1B
    Omega_0B,Omega_1B, Omega_2B = get_operator_from_y(ys, dim1B, dim2B)
    E, f , Gamma=Transformed_Ham(Omega_1B, Omega_2B, user_data)
    
    energy.append(E)
    flow.append(solver.t)
