...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 18 13:35:17 2020
...
#------------------------------------------------------------------------------
""" 
This version is based of update.py in my local Meta_Analysis folder.
I have observed that x_ph_E and y_E_ph are all zeros when sign = -1: i.e.
x = Omega_2B. Moreover, x_ph_E is also zero when sign= +1 (since x=Omega) there
as well. Knowing that many blocks of Omega are zero, but without a 
sufficient explanation for now, this version will just check that blocks are 
non-zero before doing really expensive calculations. When I'm able to predict
which blocks are zeros, I'll integrate that with the code more seemlessly.
The main changes with happen in the ph transformation and commutator routines.
...
def ph_trans_then_mult(x, y, resultant, ph_block, block_occph, idp, sign, DTYPE):
  ...
  norm_x_E_ph = LA.norm(x_E_ph)
  
  norm_y_ph_E = LA.norm(y_ph_E) 
 ...
 ##################### ph-ph block mult & inverse ##################### 
  ...
  if(norm_x_E_ph != 0.0 and norm_y_ph_E != 0.0): 
    
    print("I found a case where x_E_ph != 0.0!")
  ...
  for block_num in HH_PP_Filter:# go through blocks
    ...
    for ket in Hole_H[block_num]:#no time will be wasted looping over Hole_H[blk#]
      ...
      for bra in Particle_P[block_num] :#if Particle_P[block_num] is empty,
        ...
        norm += 8 * Argon[idp_b[block_num][bra], i2]**2 
...
#      eta1B[i, a] = -val 
  ...
  for block_num in HH_PP_Filter:# go through blocks
    ...
    temp_Gamma = Gamma[block_num]
    
    temp_eta = eta2B[block_num]
...
#        y_map_o.obj = y #set appropriate y object
        ...
        #print("fraction of zero blocks in xyi: ", 1-len(reduce_block_nums)/len(bs_len))
...
#                    Output_1B[r, r] += xyz_map[[(p,r), (p,r),block_num]] 
            ...
            for block_num in bs_len:# go through blocks    
                ...
                #I hope these temp lists are put in cache
                
                temp_xyz = xyz[block_num]
                
                temp_xyi = xyi[block_num]
                
                temp_Output_2B = Output_2B[block_num]
                ...
                for i1, (r,p) in reverse_enumerate(temp_blk,disp):
                    ...
                    i1_temp_disp = i1 + disp #I hope it's put in a register
                                      
                    Output_1B[p] +=  temp_xyz[i1, i1]
                    
                    Output_1B[r] +=  temp_xyz[i1_temp_disp , i1_temp_disp]
                    
                    ####### compute diagonal terms. See comments below. ######
                    
                    temp_Output_2B[i1, i1]-=( 
                            temp_xyi[i1, i1]
                            - temp_xyi[i1_temp_disp, i1]
                            - temp_xyi[i1, i1_temp_disp]
                            + temp_xyi[i1_temp_disp, i1_temp_disp])
                    
                    mval = temp_Output_2B[i1, i1] #I hope it's put in a register
                    temp_Output_2B[i1_temp_disp, i1]= -mval
                    temp_Output_2B[i1, i1_temp_disp]= -mval 
                    #conjugate mval in line above if using complex potential
                    temp_Output_2B[i1_temp_disp, i1_temp_disp]= mval
                        
                    #Check to make sure that every matrix element that is
                    #calculated is unique                                                          
#                    if((i1,i1) in testme or (i1_temp_disp,i1) in testme or (i1,i1_temp_disp) in testme or (i1_temp_disp,i1_temp_disp) in testme  ):
#                        print(i1, i1_temp_disp)
#                        sys.exit()
#                    testme.append((i1,i1)); testme.append((i1_temp_disp,i1))
#                    testme.append((i1,i1_temp_disp)); testme.append((i1_temp_disp,i1_temp_disp))
                    
                    del ranger[i1]#get rid of diagonal terms
                    #delete (r,p) so it does show up in Hermitian since transpose is manually coded                    
                    
                    #for i2, (j,q) in enumerate(temp_blk):
                    for i2 in ranger:  

                        i2_temp_disp = i2 + disp #I hope it's put in a register
                        
                        temp_Output_2B[i1, i2]-=( 
                                temp_xyi[i1, i2]
                                - temp_xyi[i1_temp_disp, i2]
                                - temp_xyi[i1, i2_temp_disp]
                                + temp_xyi[i1_temp_disp, i2_temp_disp])
                        
                        mval = temp_Output_2B[i1, i2] #I hope this gets put in cache
                        temp_Output_2B[i1_temp_disp, i2]= -mval
                        temp_Output_2B[i1, i2_temp_disp]= -mval
                        temp_Output_2B[i1_temp_disp, i2_temp_disp]= mval
                        
                        temp_Output_2B[i2, i1] =  mval
                        temp_Output_2B[i2, i1_temp_disp] = -mval
                        temp_Output_2B[i2_temp_disp, i1] = -mval
                        temp_Output_2B[i2_temp_disp, i1_temp_disp] = mval
                        
                        #Check to make sure that every matrix element that is
                        #calculated is unique
#                        if((i1,i2) in testme or (i1_temp_disp, i2) in testme or (i1, i2_temp_disp) in testme or (i1_temp_disp, i2_temp_disp) in testme  ):
#                            print(i1, i2, i1_temp_disp)
#                            sys.exit()
#                        if((i2,i1) in testme or (i2, i1_temp_disp) in testme or (i2_temp_disp, i1) in testme or (i2_temp_disp, i1_temp_disp) in testme  ):
#                            print(i1, i2, i1_temp_disp)
#                            sys.exit()
#                        testme.append((i1,i2)); testme.append((i1, i2_temp_disp))
#                        testme.append((i1_temp_disp, i2)); testme.append((i1_temp_disp, i2_temp_disp)) 
#                        testme.append((i2,i1)); testme.append(( i2_temp_disp, i1))
#                        testme.append((i2, i1_temp_disp)); testme.append((i2_temp_disp, i1_temp_disp))                          
            ############# Important development change ################
              
            #Two body loops split into off-diagonal and diagonal calculations 
            #because if I combine diag and off diag calculations 
            #(as in previous iterations of my) code, then I will have 
            #redundant expressions, e.g. if i1==i2: 
            #"Output_2B[block_num][i1, i1]=mval" and 
            #"Output_2B[block_num][i1, i1]=mval". If I was using a complex
            #potential, it would be "Output_2B[block_num][i1, i1]=mval", and 
            #"Output_2B[block_num][i1, i1]=np.conj(mval)" which would be 
            #wrong. So I'm just gonna split the loops. 
              
            #The off diagonal loop puts "del ranger[i1]" before 
            #"for i2 in ranger:", that way no diagonal terms are obtained.
            #See simple example below. Uncomment one of the 
            # "del y[i]" to see how the diagonals are omitted. 
                  
            #a = 5
            #y= list(range(a))#[0,1,...,a-1]
            #x = list(reversed(range(a))) #[0,1,
            ...
            reduce_block_nums = [] #don't bother with blocks that are zeros
            
            for block_num in bs_len:
              
                if LA.norm(xyi[block_num]) != 0.0:
                    
                    reduce_block_nums.append(block_num)
                
            for block_num in reduce_block_nums:# go through blocks
                ...
                for i1 in reversed(ranger):
                    ...
                    ####### compute diagonal terms separately ######
                    
                    ###### You really don't need the diag calculations below
                    #if you're using a real potential, since Omega[i,i]=0
                    #But I should check that they are actually zero. I think
                    #I did a while ago, and they were zeros. So you can just 
                    #comment everything before "for i2 in ranger:" if you want.
        ...
        for block_num in bs_len:# go through blocks  
            ...
            if(LA.norm(temp_x)!= 0.0):
                ...
                for i1, (i,j) in reverse_enumerate(Hermitian,disp):
                    ...
                    ####### look at diagonal terms separately ######
                    
                    #In mval = temp_x[i1, i1] * (y[k] + y[l] -y[i] -y[j]),
                    #if i1 = i2, then k=i, j=l, and mval = 0 
                  
                    del Hermitian[i1] #don't compute diagonals since they're zeros 
    ...
    #                    mval += y[j] * x[block_num][i1 + disp, i2]
                        ...
                        #I hope this is stored in a register
                        mval = temp_x[i1, i2] * (y[k] + y[l] -y[i] -y[j])
                        
                        #mval -= y[i,i] * x[block_num][i1, i2]
                                          
                            
                        temp_Output_2B[i1, i2] = mval 
                        temp_Output_2B[i1 + disp, i2]= -mval
                        temp_Output_2B[i1, i2 + disp]= -mval
                        temp_Output_2B[i1 + disp, i2 + disp]= mval
                        
                        temp_Output_2B[i2, i1]= mval
                        temp_Output_2B[i2, i1 + disp]= -mval
                        temp_Output_2B[i2 + disp, i1]= -mval
                        temp_Output_2B[i2 + disp, i1 + disp]= mval
...
def Inf_Matter_Ham(full_state, energy_const, Combined_consts, k_alpha, degen, L, user_data):
    ...
    for blocky in bs_len:# fill in all blocks
        ...
        for (p,q) in subset2B[blocky]: #I could use enumerate 
            ...
            ##### compute diagonals separately. See commutator routine for
            #for more comments on why I'm doing this. #####
            
            del Hermitian[Hermitian.index((p,q))] 
            #delete (p,q) so it does show up in Hermitian since transpose is manually coded 
            
            #Compute anti-symmetrized matrix element <(p,q)| H2B |(p,q)>_AS
            mval = (H2B_element(p,q,p,q,full_state,Combined_consts, k_alpha,degen,L)
            -H2B_element(p,q,q,p,full_state,Combined_consts, k_alpha,degen,L))#obtain anti-symm matrix element
            
            temp_H2B[block_loc_pq,block_loc_pq] = mval
            #Use properties of anti-symmetrized matrix elements
            temp_H2B[block_loc_pq,block_loc_qp]= -mval
            temp_H2B[block_loc_qp,block_loc_pq]= -mval
            #conjugate mval in line above if potential is complex
            temp_H2B[block_loc_qp,block_loc_qp]=  mval
...
#                    print("CM Momentum is not being conserved!")
                ...
                mval = (H2B_element(p,q,r,s,full_state,Combined_consts, k_alpha,degen,L)
                -H2B_element(p,q,s,r,full_state,Combined_consts, k_alpha,degen,L))#obtain anti-symm matrix element
                
                temp_H2B[block_loc_pq,block_loc_rs] = mval
                #Use properties of anti-symmetrized matrix elements
                temp_H2B[block_loc_pq,block_loc_sr]= -mval
                temp_H2B[block_loc_qp,block_loc_rs]= -mval
                temp_H2B[block_loc_qp,block_loc_sr]=  mval
                
                #Use Hermiticity, also note that matrix elements are real
                temp_H2B[block_loc_rs,block_loc_pq]= mval
                temp_H2B[block_loc_sr,block_loc_pq]= -mval
                temp_H2B[block_loc_rs,block_loc_qp]= -mval
                temp_H2B[block_loc_sr,block_loc_qp]= mval
...
def main(args):  
  ...
  print("Number of blocks in 2B basis: ", len(bas_block2B))
  ...
  print(" ")
  print("Number of PH Transformed Blocks of Operators: ", len(basph_block2B))
  print(" ")
  ...
  sizer = np.sum(np.array(block_sizes)**2)
  
  avg_ham_norm = [calc_full2B_norm(Gamma, bs_len)/sizer]
  ...
  avg_omega_norm.append(user_data["eta_norm"]/sizer)
  ...
  avg_ham_norm.append(calc_full2B_norm(Gamma, bs_len)/sizer)
  ...
  for flow_p in flow_pars:
    ...
    avg_omega_norm.append(calc_full2B_norm(Omega_2B, bs_len)/sizer)
  ...
  last_norm = calc_full2B_norm(Omega_2B, bs_len)/sizer #scaled norm of final omega
  
  last_ham = calc_full2B_norm(Gamma, bs_len)/sizer#scaled norm of final H2B
  ...
  results.append([E, 0, 0 , Omega_2B, frac, diff, 
                  dim1B, last_norm ,
                  np.mean(np.array(avg_omega_norm)), last_ham, np.mean(np.array(avg_ham_norm))  ])#don't need f & Gamma right now    
