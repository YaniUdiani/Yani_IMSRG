...
# -*- coding: utf-8 -*-
"""
Created on Sun Feb  9 15:59:59 2020
...
# 
#This version is based off Exp3_Upgrade1.py (a good benchmark)
#It allows the user to turn off the computation of Omega1B for plane wave 
#basis. This shows marginal improvements in the smaller Nmax regions 
#As expected, but there should be some gains made for higher N_Max values
#------------------------------------------------------------------------------
...
parser = argparse.ArgumentParser(description='Arguments for Inf Matter IMSRG')
...
parser.add_argument("--g", required=True, type=int, choices=[2,4],
...
parser.add_argument("--T", required=False, type=int, choices=range(1,17), default=3, 
                    help="BCH expansion will go up to T terms")
parser.add_argument("--Inf", required=False, type=int, choices=[0,1],
                    default= 0, help="=0 for Omega1B set to 0, & =1 for Omega1B not set to 0")

args = parser.parse_args() #obtain arguments from command line
#-----------------------------------------------------------------------------------
# Ploting variables
#-----------------------------------------------------------------------------------
...
def engine1_iteration(occphA_2B,blk_nums_that_matter,subset2B,idp, subset_size,bs_len):
  
  """" Simplifies ph_engine1 so that it only finds (a,d),(c,b),etc.. in the same blk """
  dicty = {}#used to store simplified list
  counter = 0
  naive_nums = 0
  store = {} #for a given BN and block_num, which blks are full?
  for row in range(len(occphA_2B)):
    BN = blk_nums_that_matter[row]
    disp_1 = subset_size[BN]# = to how many pairs away (j,i) is from (i,j)
    
    for block_num in blk_nums_that_matter:
      if (BN,block_num) not in dicty.keys():#set locations for eng_1_loop[BN,block_num]
        dicty[(BN,block_num)] = [[] for i in bs_len]
        store[(BN,block_num)] = []
      disp_2 = subset_size[block_num]# = to how many pairs away (j,i) is from (i,j)
      naive_nums +=len(subset2B[BN]) * len(subset2B[block_num])
      
      for i1,(a,b) in enumerate(subset2B[BN]):
        for i2,(c,d) in enumerate(subset2B[block_num]):   
          
          if(a!=d and b!=c):#exclusion principle
            first = idp[(a,d)]
            second = idp[(c,b)]
            if(first[0] == second[0]):#if they're in same blk, store
              counter+=1
              first1 = idp[(d,a)][1]
              second1 = idp[(b,c)][1]
              dicty[(BN,block_num)][first[0]].append([i1, i2, first[1], second[1]])
              dicty[(BN,block_num)][first[0]].append([i1 + disp_1, i2 + disp_2, second1, first1])

          if(b!=d and a!=c): #exclusion principle
            first = idp[(b,d)]
            second = idp[(c,a)]
            if(first[0] == second[0]):#if they're in same blk, store
              counter+=1
              first1 = idp[(a,c)][1]
              second1 = idp[(d,b)][1]
              dicty[(BN,block_num)][first[0]].append([i1 + disp_1, i2, first[1], second[1]])
              dicty[(BN,block_num)][first[0]].append([i1, i2 + disp_2, first1, second1])
      del_values = []        
      for i in bs_len:#convert to arrays (should be better for Cython)
        if(dicty[(BN,block_num)][i]): #if blk num has pairs that contribute
          store[(BN,block_num)].append(i)#Non-empty blks
          dicty[(BN,block_num)][i] = np.array(dicty[(BN,block_num)][i])
        else:
          del_values.append(i)#store empty blks to be deleted
          
      for i in sorted(del_values, reverse=True):
        del dicty[(BN,block_num)][i] #remove empty lists
        
  print("Fraction of all pairs that contribute to ph_engine1: ", round(counter/naive_nums,8))      
  return dicty, store
  
def engine2_iteration(bas2B, blocks_that_matter,idp, bs_len):
  
  """" Simplifies ph_engine2 so that it only finds (a,d),(c,b) in the same blk """
  counter=0 #get statistics
  iterations = [[] for i in bs_len]
  store = [] #which blks are full?
  for i1, (a,b) in enumerate(bas2B):
    for i2, (c, d) in enumerate(blocks_that_matter):#only use columns that matter to construct x_ph
      if(a!=d and c!=b): #exclusion principle
        first = idp[(a,d)]
        second = idp[(c,b)]
        if(first[0] == second[0]):#if they're in same blk, store
          counter+=1
          iterations[first[0]].append([i1, i2, first[1], second[1]])#store
        
  for i in bs_len:#convert to arrays (should be better for Cython)
    if(iterations[i]): #if blk num has pairs that contribute
      store.append(i)
      iterations[i] = np.array(iterations[i])
      
  print("Fraction of all pairs that contribute to ph_engine2: ", round(counter/len(bas2B)/len(blocks_that_matter),8))
  
  return iterations,store
  

def ph_engine1(y_ph_blk,y_blk,dict_elem):
  """ Turns out this is the 2nd most expensive piece of PH_Trans """
  for element in dict_elem:
    y_ph_blk[element[0], element[1]] -=y_blk[element[2], element[3]]
  return

def ph_engine2(x_ph,x_blk,eng_2_loop_blk):
  """ Turns out this is the most expensive piece of PH_Trans """
  for element in eng_2_loop_blk:
    x_ph[element[0], element[1]] -= x_blk[element[2], element[3]]
  return
      
def ph_transform_2B(x,y,occphA_2B,
                    bas2B,bs_len,subset2B,
                    blocks_that_matter,blk_nums_that_matter,y_ph_zeros,
                    eng_2_loop,dicty,store1):

  List_Prod=[]
  append = List_Prod.append
  ...
  for row in range(len(occphA_2B)): #get PH transformation on y for each row that matters in Occ*ph(Y)
      ...
      for block_num in blk_nums_that_matter: #I'm looping over blk_nums_that_matter 
          ...
          #disp_2 = subset_size[block_num]# = to how many pairs away (j,i) is from (i,j)
          for (index,blockys) in enumerate(store1[(BN,block_num)]):
              ph_engine1(y_ph[block_num],y[blockys],dicty[(BN,block_num)][index])
      ...
      append(y_ph) #append new row of concatenated block matrices to List_Prod
  ...
  for block_num in bs_len:
    if(type(eng_2_loop[block_num] != list)): #if there terms to add for a given blk
      ph_engine2(x_ph,x[block_num],eng_2_loop[block_num])
...
#commutator must be such that ~[H,Omega] returns a hermitian matrix; and ~[Omega, eta] returns an anti-hermitian matrix
    ...
    holes     = user_data["holes"]
    particles = user_data["particles"]
    Omega1B_Eq_Zero = user_data["Omega1B_Eq_Zero"]#if 0 then, Omega1B= zeros, else calculate it!
    ...
    if(type(x) == np.ndarray and type(y) == np.ndarray):#1B-1B
        ...
        if(Omega1B_Eq_Zero == 1 or sign == +1):
    ...
    if(type(x) == list and type(y) == np.ndarray):#2B-1B 
        ...
        if(Omega1B_Eq_Zero == 1 or sign == +1):#only Omega1B =0, not H1B
    ...
    if(type(x) == list and type(y) == list):# 2B-2B
        ...
        eng2l = user_data["eng_2_loop"] 
        #store2 = user_data["store2"]
        dicty = user_data["dicty"]
        store1 = user_data["store1"]
        ...
        xy_ph = ph_transform_2B(x,y,occphA_2B,
                                bas2B,bs_len,subset2B,
                                btm,bntm,ypz,eng2l,dicty,store1)
        ...
        else: #only worry about 2BD piece
            for block_num in bs_len:# go through blocks
...
def main():
  ...
  DE_flag=args.DE
  trunc = args.T
  Omega1B_Eq_Zero = args.Inf # =0: Set Omega1B=0 always- using plane wave basis. =1: You're not using plane wave/other reason
  ...
  eng_2_loop,store2 = engine2_iteration(bas2B, blocks_that_matter,idp, bs_len)#smaller loop range for ph_engine2
  dicty,store1 = engine1_iteration(occphA_2B,blk_nums_that_matter,subset2B,idp, subset_sizes,bs_len)#smaller loop range for ph_engine1
  bn2 = Bernoulli_generator(trunc)# Go up to trunc terms in expansion
  ...
  user_data  = {
    ...
    "states": states,
    "Omega1B_Eq_Zero": Omega1B_Eq_Zero
    #"OP_Map": OP_Map
  }
